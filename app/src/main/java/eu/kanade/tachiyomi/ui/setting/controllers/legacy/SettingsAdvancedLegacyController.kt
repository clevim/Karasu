...
class SettingsAdvancedLegacyController : SettingsLegacyController() {
    ...
    override fun setupPreferenceScreen(screen: PreferenceScreen) = screen.apply {
        ...
        preference {
            ...
            summaryMRes = MR.strings.saves_error_logs
        ...
        switchPreference {
            ...
            titleMRes = MR.strings.pref_verbose_logging
            summaryMRes = MR.strings.pref_verbose_logging_summary
        }

        preference {
            key = "debug_info"
            titleMRes = MR.strings.pref_debug_info
        ...
        preferenceCategory {
            titleMRes = MR.strings.label_background_activity
            val pm = context.getSystemService(Context.POWER_SERVICE) as? PowerManager?
            if (pm != null) {
                preference {
                    key = "disable_batt_opt"
                    titleMRes = MR.strings.disable_battery_optimization
                    summaryMRes = MR.strings.disable_if_issues_with_updating
            ...
            preference {
                ...
                summaryMRes = MR.strings.about_dont_kill_my_app
        ...
        if (isUpdaterEnabled) {
            switchPreference {
                titleMRes = MR.strings.check_for_beta_releases
                summaryMRes = MR.strings.try_new_features
        ...
        preferenceCategory {
            titleMRes = MR.strings.data_management

            preference {
                titleMRes = MR.strings.force_download_cache_refresh
                summaryMRes = MR.strings.force_download_cache_refresh_summary
                onClick { downloadManager.refreshCache() }
            }

            preference {
                key = "clean_downloaded_chapters"
                titleMRes = MR.strings.clean_up_downloaded_chapters

                summaryMRes = MR.strings.delete_unused_chapters

                onClick {
                    activity!!.materialAlertDialog()
                        .setTitle(MR.strings.clean_up_downloaded_chapters)
                        .setMultiChoiceItems(
                            eu.kanade.tachiyomi.R.array.clean_up_downloads,
            ...
            preference {
                ...
                titleMRes = MR.strings.pref_clear_webview_data

                onClick { clearWebViewData() }
            }
            preference {
                key = "clear_database"
                titleMRes = MR.strings.clear_database
                summaryMRes = MR.strings.clear_database_summary
                onClick { router.pushController(ClearDatabaseController().withFadeTransaction()) }
            }
        }

        preferenceCategory {
            titleMRes = MR.strings.network
            preference {
                key = "clear_cookies"
                titleMRes = MR.strings.clear_cookies
            ...
            editTextPreference(activity) {
                ...
                titleMRes = MR.strings.user_agent_string
        ...
        preferenceCategory {
            ...
            listPreference(activity) {
                ...
                titleMRes = MR.strings.ext_installer_pref

                val values = BasePreferences.ExtensionInstaller.entries.toList()
                        ...
                        return@onChange if (!context.isPackageInstalled(ShizukuInstaller.Companion.shizukuPkgName) && !Sui.isSui()) {
                            context.materialAlertDialog()
                                ...
                                .setPositiveButton(MR.strings.download) { _, _ ->
                                    openInBrowser(ShizukuInstaller.Companion.downloadLink)
                                }
                                .setNegativeButton(R.string.cancel, null)
                                .show()
                            false
            ...
            preference {
                titleMRes = MR.strings.action_revoke_all_extensions
        ...
        preferenceCategory {
            titleMRes = MR.strings.library
            preference {
                key = "refresh_lib_meta"
                titleMRes = MR.strings.refresh_library_metadata
                summaryMRes = MR.strings.updates_covers_genres_desc

                onClick { LibraryUpdateJob.Companion.startNow(context, target = LibraryUpdateJob.Target.DETAILS) }
            }
            preference {
                key = "refresh_teacking_meta"
                titleMRes = MR.strings.refresh_tracking_metadata
                summaryMRes = MR.strings.updates_tracking_details
        ...
        preferenceCategory {
            titleMRes = MR.strings.reader

            intListPreference(activity) {
                bindTo(basePreferences.hardwareBitmapThreshold())
                titleMRes = MR.strings.pref_hardware_bitmap_threshold
            ...
            switchPreference {
                ...
                titleMRes = MR.strings.pref_reader_debug_mode
            }
        }

        preferenceCategory {
            titleMRes = MR.strings.local_source
    ...
    private fun clearWebViewData() {
        ...
        try {
            ...
            Logger.Companion.e(e) { "Unable to delete WebView data" }
